...
```
storage.js → levels.js → engine.js → race.js → fx.js → ui.js → main.js
```
...
- **`race.js`** — `TG.race.createRace(trackEl, level)` → `{update, pulse}`. The visual mini-game: player vehicle position tracks typed progress; a rival vehicle paces at the level's `minWpm` threshold. Fed by the engine's `onProgress` (position) and `onCharResult` (boost/skid pulse).
